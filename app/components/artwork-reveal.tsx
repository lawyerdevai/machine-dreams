...
/**
 * Sequential post-creation unveil (bio → artwork → title → statement → status).
 * Shared by /test-create and /artwork/[tokenId]?justCreated=true.
 */
export function ArtworkReveal({
  ...
  /** Rendered when the status stage begins (live mint block or preview shell). */
  statusSlot: ReactNode;
  /** Fired once the full unveil settles (real page swaps to normal catalog). */
  onComplete?: () => void;
}) {
  ...
  useEffect(() => {
    if (stage !== "done" || completedRef.current) return;
...
/** Disabled STATUS/MINT shell for /test-create preview. */
export function PreviewStatusSlot() {
  return (
